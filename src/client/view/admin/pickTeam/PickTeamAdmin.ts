...
import { syncWorldWarPanel3, getLowerthird, getPlayerMap } from '../../utils/HupuAPI';
...
export class _PickTeamAdmin extends VueBase {
    ...
    updataPlayerMap() {
        ...
        getPlayerMap(doc => {
            this.confType2_playerMap
            // let doc = data.event_1.doc
            console.log('playerMap', doc.playerMap)
            this.confType2_playerMap = []
            for (let p of doc.player_all) {
                let pid = p.player_id
    ...
    methods = {
        ...
        onShowSelectedPlayerType(type) {
            let clonePlayer = clone(this.selected_player)
